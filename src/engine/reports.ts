...
export function toJson(findings: Finding[], score: number, fileCount: number, scannedAt = new Date().toISOString()): string {
  return JSON.stringify({ tool: 'SAYANOX FORGE', version: VERSION, scannedAt, healthScore: score, filesScanned: fileCount, findings }, null, 2);
}
...
export function toHtml(findings: Finding[], score: number, fileCount: number): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="generator" content="SAYANOX FORGE ${VERSION}"><title>SAYANOX FORGE Report</title><style>body{font-family:system-ui,sans-serif;max-width:1200px;margin:40px auto;padding:0 20px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#f5f5f5}.score{font-size:2rem;font-weight:700}</style></head><body><h1>SAYANOX FORGE Scan Report</h1><p class="score">${score}/100</p><p>${fileCount} files scanned · ${findings.length} findings</p><table><thead><tr><th>Severity</th><th>Rule</th><th>Title</th><th>Location</th><th>Message</th></tr></thead><tbody>${rows || '<tr><td colspan="5">No findings detected.</td></tr>'}</tbody></table></body></html>`;
}
